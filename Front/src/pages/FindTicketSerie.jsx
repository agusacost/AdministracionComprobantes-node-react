...
export const FindTicketSerie = () => {
    const {getTicketBySerie} = useTickets();
    const [ticketSerie, setTicketSerie] = useState('');
    ...
    const handleSubmit = async (event) =>{
        ...
        try {
          const response = await getTicketBySerie(ticketSerie);
          setResult(response);
          setError('')
        } catch (error) {
          setError('No se encuentra ticket asociado a la serie');
          setResult(null)
        }
    }
  ...
  return (
    <div className="max-w-md mx-auto p-4 mt-24 mb-96">
          <form onSubmit={handleSubmit} className="flex flex-col items-center">
            ...
            <label className="mr-2">Numero de Serie: </label>
            <input
              type="text"
              placeholder="Ingrese el numero de serie"
              value={ticketSerie}
              onChange={(e)=>setTicketSerie(e.target.value)}
...
export default FindTicketSerie;
